...
int main(int argc, char *argv[])
{
    ...
    if (dumping) {
        /// modification
        GFile binFile(output);
        if (binFile.fileSuffix() == "lsc" && binFile.exists() && binFile.open(GFile::ReadOnly | GFile::Binary)) {
            GByteArray rawData = binFile.read();
            binFile.close();

            GByteArray tarData = GByteArray::compress(rawData);
